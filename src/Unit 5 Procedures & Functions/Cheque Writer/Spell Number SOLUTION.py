...
#TAKES A 3-DIGIT NUMBER (REPRESENTED AS A STRING) AS A PARAMETER AND RETURNS THE ENGLISH SPELLING OF THAT NUMBER
...
#### RECURSIVE VERSION ##########3
def spellAnyNumber_RECURSIVE( num ):
    ...
    if d <= 3:
        ...
        indexOfSuffix = floor( (d-1)/3 ) - 1
        suffix = suffixes[ indexOfSuffix ]
        
        if d % 3 == 0:
            numLeadingDigits = 3
        else:
            numLeadingDigits = d % 3
            
        leadingDigits = num[0:numLeadingDigits]
        remainingDigits = num[ numLeadingDigits: len(num) ]
        print(suffix, numLeadingDigits, leadingDigits,remainingDigits)

        if numLeadingDigits == 1:
            leadingPhrase = spellOnes( leadingDigits ) + " " + suffix
            
        if numLeadingDigits == 2:
            leadingPhrase = spellTwoDigits( leadingDigits ) + " " + suffix
            
        if numLeadingDigits == 3:
            leadingPhrase = spellThreeDigits( leadingDigits ) + " " + suffix

    return leadingPhrase + " " + spellAnyNumber_RECURSIVE( remainingDigits )
...
def speller( num ):
    ...
    if L <= 3:
        ...
        for b in range(1, numBlocks+1 ):
            ...
            if b < numBlocks: #all blocks except the last one have a suffix

                spellingOfThisBlock = threeDigitSpeller( currentBlock )

                if spellingOfThisBlock != "":
                    spelling = spelling + " " + threeDigitSpeller( currentBlock ) + " " + endings[ endingsIndex ]

            else: #the last block has no suffix
